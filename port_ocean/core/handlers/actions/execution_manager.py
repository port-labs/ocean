        ...
        async with event_context(
            ...
            with logger.contextualize(run_id=run.id, action=run.action_type):
                try:
                    ...
                    while (
                        await executor.is_close_to_rate_limit(run)
                        and not self._is_shutting_down.is_set()
                    ):
                        backoff_seconds = min(
                            RATE_LIMIT_MAX_BACKOFF_SECONDS,
                            await executor.get_remaining_seconds_until_rate_limit(run),
                        )
                        logger.info(
                ...
                error_status_label: str = DEFAULT_FAILURE_STATUS_LABEL
                try:
                    ...
                    error_summary = str(e)
                    if e.status_label:
                        error_status_label = e.status_label
                except Exception as e:
