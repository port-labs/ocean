...
def client(
    ...
    monkeypatch.setattr(router_module, "_require_provider", lambda _: mock_provider)
...
def test_authorize_rejects_an_unconfigured_target(
    ...
    monkeypatch.setattr(
        router_module,
        "_require_provider",
        lambda _: (_ for _ in ()).throw(HTTPException(status_code=404, detail="not found")),
    )
...
def test_callback_stores_the_token_and_resumes_the_run(
    ...
    state = sign_state("run_1", "wfnr_1", "jane@acme.com", "org_1", "github")
    ...
    assert response.status_code == 302
    assert response.headers["location"] == "https://app.getport.io/runs/run_1"
    ...
    assert (org_id, actor_id, target) == ("org_1", "jane@acme.com", "github")
