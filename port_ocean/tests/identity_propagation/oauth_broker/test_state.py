...
def test_a_signed_state_round_trips() -> None:
    payload = verify_state(
        sign_state("run_1", "wfnr_1", "jane@acme.com", "org_1", "github")
    )
...
def test_a_tampered_payload_is_rejected() -> None:
    encoded, _, signature = sign_state(
        "run_1", "wfnr_1", "jane@acme.com", "org_1", "github"
    ).partition(".")
    forged = sign_state(
        "run_1", "wfnr_1", "attacker@evil.com", "org_1", "github"
    ).split(".")[0]
...
def test_an_expired_state_is_rejected() -> None:
    state = sign_state(
        "run_1", "wfnr_1", "jane@acme.com", "org_1", "github", ttl_seconds=-1
    )
