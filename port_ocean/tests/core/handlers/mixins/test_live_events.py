...
async def test_sync_raw_results_groups_entities_by_blueprint(
    ...
    first_call_entities = call_args_list[0][0][
        0
    ]  # First positional argument of first call
    second_call_entities = call_args_list[1][0][
        0
    ...
    assert (
        len(first_call_blueprints) == 1
    ), "First call should contain entities from only one blueprint"
    assert (
        len(second_call_blueprints) == 1
    ), "Second call should contain entities from only one blueprint"
    ...
    service_entities_processed = [
        e for e in all_processed_entities if e.blueprint == "service"
    ]
    deployment_entities_processed = [
        e for e in all_processed_entities if e.blueprint == "deployment"
    ]
