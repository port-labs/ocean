...
class TestJQEntityProcessor:
    ...
    async def test_compile(
        self, monkeypatch: Any, mocked_processor: JQEntityProcessor
    ) -> None:
    ...
    async def test_search_as_object(
        self, monkeypatch: Any, mocked_processor: JQEntityProcessor
    ) -> None:
        with monkeypatch.context():
            data = {"foo": {"bar": "baz"}}
    ...
    async def test_get_mapped_entity(
        self, monkeypatch: Any, mocked_processor: JQEntityProcessor
    ) -> None:
        with monkeypatch.context():
            data = {"foo": "bar"}
    ...
    async def test_calculate_entity(
        self, monkeypatch: Any, mocked_processor: JQEntityProcessor
    ) -> None:
        with monkeypatch.context():
            data = {"foo": "bar"}
    ...
    async def test_parse_items_performance_10000(
        ...
        with monkeypatch.context():
            ...
            for _ in range(10000):
                ...
                assert len(result.entity_selector_diff.passed) == 1
                assert (
                    result.entity_selector_diff.passed[0].properties.get("foo") == "bar"
                )
                assert not result.errors
