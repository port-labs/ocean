...
class SessionManager:
    ...
    async def get_session(
        self, region: Optional[str] = None, role_arn: Optional[str] = None
    ) -> aioboto3.Session:
            ...
            for attempt in range(self._max_retries):
                try:
                    ...
                    if attempt == self._max_retries - 1:
                        logger.error(f"Failed to create session after {self._max_retries} attempts: {e}")
                        raise
        ...
        async with self._lock:
            ...
            for attempt in range(self._max_retries):
                try:
                    ...
                    if attempt == self._max_retries - 1:
                        logger.error(f"Failed to create client after {self._max_retries} attempts: {e}")
                        raise
                    await asyncio.sleep(self._retry_delay * (attempt + 1))
