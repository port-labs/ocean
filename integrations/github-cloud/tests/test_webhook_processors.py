...
async def test_workflow_webhook_processor_should_process_event(mock_workflow_run_event):
    processor = WorkflowWebhookProcessor(mock_workflow_run_event)
    assert await processor.should_process_event(mock_workflow_run_event) is True
...
def test_workflow_webhook_processor_events():
    ...
    assert "workflow_job" in events

@pytest.mark.asyncio
async def test_workflow_webhook_processor_handle_workflow_run_event(mock_workflow_run_event):
    processor = WorkflowWebhookProcessor(mock_workflow_run_event)
    mock_github_client = AsyncMock()
    processor._github_cloud_webhook_client = mock_github_client

    # Mock the workflow run data
    mock_run = {
        "id": 789,
        "name": "Test Workflow",
        "status": "completed",
        ...
        "repository": mock_workflow_run_event.payload["repository"]
    }
    mock_github_client.get_single_workflow_run.return_value = mock_run

    # Mock the workflow jobs iterator
    mock_jobs = [
        {
            "id": 456,
            "name": "Test Job",
    ...
    async def mock_get_workflow_jobs(*args, **kwargs):
        yield mock_jobs

    mock_github_client.get_workflow_jobs = mock_get_workflow_jobs

    # Test handle_event
    result = await processor.handle_event(mock_workflow_run_event.payload, MagicMock())

    # Verify the result
    assert len(result.updated_raw_results) == 1
    updated_run = result.updated_raw_results[0]
    assert updated_run["id"] == 789
    assert updated_run["name"] == "Test Workflow"
    assert "jobs" in updated_run
    assert len(updated_run["jobs"]) == 1
    assert updated_run["jobs"][0]["id"] == 456

    # Verify the mock was called correctly
    mock_github_client.get_single_workflow_run.assert_called_once_with("owner/test-repo", 789)

@pytest.mark.asyncio
async def test_workflow_webhook_processor_handle_workflow_job_event(mock_workflow_job_event):
    processor = WorkflowWebhookProcessor(mock_workflow_job_event)
    mock_github_client = AsyncMock()
    processor._github_cloud_webhook_client = mock_github_client

    # Mock the workflow job data
    mock_job = {
        "id": 456,
        "name": "Test Job",
        "status": "completed",
        ...
        "repository": mock_workflow_job_event.payload["repository"]
    }
    mock_github_client.get_single_workflow_job.return_value = mock_job
    ...
    # Test handle_event
    result = await processor.handle_event(mock_workflow_job_event.payload, MagicMock())

    # Verify the result
    assert len(result.updated_raw_results) == 1
    updated_job = result.updated_raw_results[0]
    assert updated_job["id"] == 456
    assert updated_job["name"] == "Test Job"
    assert "workflow_run" in updated_job
    assert updated_job["workflow_run"]["id"] == 789

@pytest.mark.asyncio
async def test_workflow_webhook_processor_handle_event_fetch_failed(mock_workflow_run_event):
    processor = WorkflowWebhookProcessor(mock_workflow_run_event)
    mock_github_client = AsyncMock()
    processor._github_cloud_webhook_client = mock_github_client

    # Mock failed workflow run fetch
    mock_github_client.get_single_workflow_run.return_value = None

    # Test handle_event
    result = await processor.handle_event(mock_workflow_run_event.payload, MagicMock())

    # Should use the workflow run from the payload
    expected_run = mock_workflow_run_event.payload["workflow_run"]
    expected_run["repository"] = mock_workflow_run_event.payload["repository"]

    assert result.updated_raw_results == [expected_run]
    assert result.deleted_raw_results == []
    mock_github_client.get_single_workflow_run.assert_called_once_with(
        "owner/test-repo", 789
    )

@pytest.mark.asyncio
async def test_workflow_webhook_processor_validate_payload_missing_workflow_run():
    event = WebhookEvent(
...
@pytest.mark.asyncio
async def test_workflow_webhook_processor_validate_payload_missing_workflow_job():
    event = WebhookEvent(
