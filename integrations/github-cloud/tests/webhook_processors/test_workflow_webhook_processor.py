...
from github_cloud.webhook_processors.workflow_webhook_processor import (
...
async def processor():
    with patch(
        "github_cloud.webhook_processors.workflow_webhook_processor.init_client"
    ) as mock_init_client:
        mock_client = AsyncMock()
        mock_init_client.return_value = mock_client
        processor = WorkflowWebhookProcessor()
        yield processor


@pytest.fixture
def valid_workflow_event():
    return WebhookEvent(
        payload={
            "action": "created",
...
@pytest.mark.asyncio
async def test_should_process_event(processor, valid_workflow_event):
    # Test valid event
    assert await processor.should_process_event(valid_workflow_event) is True
...
@pytest.mark.asyncio
async def test_get_supported_resource_kinds(processor, valid_workflow_event):
    kinds = await processor.get_supported_resource_kinds(valid_workflow_event)
...
async def test_validate_payload(processor):
    ...
    invalid_payload = {"action": "created", "workflow": {"name": "test-workflow"}}
...
@pytest.mark.asyncio
async def test_process_webhook_event(processor, valid_workflow_event):
    # Mock the client's fetch_resource method
    processor.client.fetch_resource.return_value = {"name": "test-workflow", "id": 1}

    # Test successful event processing
    result = await processor.process_webhook_event(valid_workflow_event.payload, {})
