...
from github_cloud.webhook_processors.repository_webhook_processor import (
...
async def processor():
    with patch(
        "github_cloud.webhook_processors.repository_webhook_processor.init_client"
    ) as mock_init_client:
...
@pytest.mark.asyncio
async def test_should_process_event(processor, valid_repo_event):
    # Test valid event
    assert await processor.should_process_event(valid_repo_event) is True
...
@pytest.mark.asyncio
async def test_get_supported_resource_kinds(processor, valid_repo_event):
    kinds = await processor.get_supported_resource_kinds(valid_repo_event)
...
async def test_validate_payload(processor):
    ...
    invalid_payload = {"action": "created", "repository": {"name": "test-repo"}}
...
@pytest.mark.asyncio
async def test_process_webhook_event(processor, valid_repo_event):
    # Mock the client's fetch_resource method
    processor.client.fetch_resource.return_value = {
        "name": "test-repo",
        "full_name": "org/test-repo",
    }

    # Test successful event processing
    result = await processor.process_webhook_event(valid_repo_event.payload, {})
