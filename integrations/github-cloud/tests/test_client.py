...
async def test_create_webhooks_if_not_exists_success(mock_client):
    ...
    mock_client.http_client.post.return_value = {
        "data": {"createWebhook": {"webhook": {"id": "123"}}}
    }
            ...
            "organization": {
                "repositories": {
                    "nodes": [
                        {
                            "webhooks": {
                                "nodes": [{"url": "https://example.com/webhook"}]
                            }
                        }
                    ]
                }
...
async def test_get_organizations(mock_client):
    mock_client.http_client.get.return_value = {
        "data": {"viewer": {"organizations": {"nodes": [{"login": "org1"}]}}}
    }
...
async def test_get_pull_requests(mock_client):
    mock_client.http_client.get.return_value = {
        "data": {"repository": {"pullRequests": {"nodes": [{"number": 1}]}}}
    }
...
async def test_get_teams(mock_client):
    mock_client.http_client.get.return_value = {
        "data": {"organization": {"teams": {"nodes": [{"slug": "team1"}]}}}
    }
...
async def test_get_workflows(mock_client):
    mock_client.http_client.get.return_value = {
        "data": {"repository": {"workflows": {"nodes": [{"id": "123"}]}}}
    }
    workflows = [
        workflow async for workflow in mock_client.get_workflows("org1", "repo1")
    ]
...
async def test_get_single_resource(mock_client):
    ...
    result = await mock_client.get_single_resource(
        "pull_requests", "1", "org1", "repo1"
    )
...
async def test_handle_rate_limit(mock_client):
    mock_client.http_client.get.return_value = {
        "data": {"rateLimit": {"remaining": 0, "resetAt": "2024-01-01T00:00:00Z"}}
    }
    with pytest.raises(Exception):
        await mock_client._handle_rate_limit(
            {"data": {"rateLimit": {"remaining": 0, "resetAt": "2024-01-01T00:00:00Z"}}}
        )
