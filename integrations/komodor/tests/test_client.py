...
async def test_get_all_services(mock_komodor_client: KomodorClient) -> None:
    ...
    api_response = {"data": {"services": page_data}, "meta": {"page": 0, "page_size": NUM_OF_PAGES}}
...
@pytest.mark.asyncio
async def test_get_all_services_multiple_pages(mock_komodor_client: KomodorClient) -> None:
    pages = await generate_service_response(NUM_OF_PAGES)
    ...
    with patch.object(
        ...
        assert len(services) == NUM_OF_PAGES
        assert services == [page["services"] for entry in pages for page in entry["data"]]
