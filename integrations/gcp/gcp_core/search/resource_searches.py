...
async def get_single_project(
    project_name: str, matching_resource_configs: list[Any] = None
) -> RAW_ITEM:
    async with ProjectsAsyncClient() as projects_client:
...
async def get_single_folder(
    folder_name: str, matching_resource_configs: list[Any] = None
) -> RAW_ITEM:
    async with FoldersAsyncClient() as folders_client:
        return parse_protobuf_message(
...
async def get_single_organization(
    organization_name: str, matching_resource_configs: list[Any] = None
) -> RAW_ITEM:
    async with OrganizationsAsyncClient() as organizations_client:
        return parse_protobuf_message(
...
async def get_single_topic(
    project_id: str, topic_id: str, matching_resource_configs: list[Any] = None
) -> RAW_ITEM:
    """
...
async def get_single_subscription(
    project_id: str, subscription_id: str, matching_resource_configs: list[Any] = None
) -> RAW_ITEM:
    """
...
async def feed_event_to_resource(
    ...
    matching_resource_configs: list[Any] = None,
) -> RAW_ITEM:
    resource = None
    if asset_data.get("deleted") is True:
        ...
        resource[EXTRA_PROJECT_FIELD] = await get_single_project(
            project_id, matching_resource_configs
        )
    else:
        match asset_type:
            case AssetTypesWithSpecialHandling.TOPIC:
                ...
                resource = await get_single_topic(
                    project_id, topic_name, matching_resource_configs
                )
                resource[EXTRA_PROJECT_FIELD] = await get_single_project(
                    project_id, matching_resource_configs
                )
