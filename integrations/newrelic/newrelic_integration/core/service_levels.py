...
class ServiceLevelsHandler:
    ...
    async def get_service_level_indicator_value(
        ...
        query = await render_query(
            GET_SLI_BY_NRQL_QUERY, nrql_query=nrql, account_id=ocean.integration_config.get("new_relic_account_id")
        )
        response = await send_graph_api_request(
            http_client,
            query,
            request_type="get_service_level_indicator_value"
        )
        service_levels = response.get("data", {}).get("actor", {}).get("account", {}).get("nrql", {}).get("results", [])
        if service_levels:
    ...
    async def list_service_levels(
        ...
        async for service_level in send_paginated_graph_api_request(
            ...
            nrql = service_level.get("serviceLevel", {}).get("indicators", [])[0].get("resultQueries", {}).get("indicator", {}).get("nrql")
            service_level[SLI_OBJECT] = await self.get_service_level_indicator_value(self.http_client, nrql)
