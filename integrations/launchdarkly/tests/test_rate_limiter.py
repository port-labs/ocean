...
async def test_handle_rate_limit_error_logic(
    ...
    """
    reset_time_in_header = int((time.time() + 10) * 1000)
    headers = {
        "X-Ratelimit-Route-Limit": "10",
        "X-Ratelimit-Route-Remaining": "0",
        "X-Ratelimit-Reset": str(reset_time_in_header),
    }
    mock_response = create_autospec(httpx.Response, instance=True)
    mock_response.headers = headers
    mock_response.status_code = 429

    rate_limiter = LaunchDarklyRateLimiter(max_retries=max_retries)
    rate_limiter._retries = retries

    with patch("time.time", return_value=(reset_time_in_header / 1000) - 10):
    ...
    if should_retry:
        ...
        assert abs(sleep_duration - 10.5) < 0.01
    else:
        mock_sleep.assert_not_awaited()
