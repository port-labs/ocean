...
_ON_LAUNCHDARKLY_RETRY_CALLBACK: Callable[[httpx.Request], httpx.Request] | None = None
...
class LaunchDarklyRetryTransport(RetryTransport):
        ...
        while True:
            ...
            try:
                ...
                if remaining_attempts < 1:
                    self._log_error(request, error)
                    raise
            if _ON_LAUNCHDARKLY_RETRY_CALLBACK:
                request = _ON_LAUNCHDARKLY_RETRY_CALLBACK(request)
            attempts_made += 1
    ...
    def _calculate_sleep(
        ...
        # The X-Ratelimit-Reset response HTTP header indicates how long the user agent should wait before
        # making a follow-up request.
        # When sent with a 429 (Too Many Requests) response, this indicates how long to wait before
        # making a new request.
        retry_after_header = (headers.get("X-Ratelimit-Reset") or "").strip()
