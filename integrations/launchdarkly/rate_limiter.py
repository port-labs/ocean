...
class LaunchDarklyRateLimiter:
    """
    Orchestrates LaunchDarkly API requests to handle rate limits gracefully.

    This class provides a context manager to handle rate-limiting. It inspects
    LaunchDarkly's rate-limiting headers on each response and introduces delays to
    prevent hitting the rate limit. It handles both proactive (based on
    X-Ratelimit-Global-Remaining) and reactive (based on 429 Too Many Requests
    with a Retry-After header) rate limiting.

    LaunchDarkly Rate Limit Headers:
    - X-Ratelimit-Global-Remaining: The number of requests remaining in the window.
    - X-Ratelimit-Reset: The UTC epoch timestamp when the window resets.

    For more information, see: https://launchdarkly.com/docs/api#rate-limiting

    Usage:
        limiter = LaunchDarklyRateLimiter()
        client = httpx.AsyncClient()
        async with limiter:
            response = await client.get("https://app.launchdarkly.com/api/...")
    """
    ...
    def __init__(
        ...
        Args:
            ...
            concurrent_requests (int): The maximum number of coroutines that
                can simultaneously access the LaunchDarkly API.
        """
    ...
    async def _update_rate_limit_state(self, response: httpx.Response) -> None:
        ...
        async with self._lock:
            remaining = headers.get("X-Ratelimit-Global-Remaining")
            ...
            if remaining is not None and reset is not None:
                logger.debug(
                    f"LaunchDarkly rate limit state: "
