...
from aws.core.exporters.codedeploy import CodeDeployDeploymentExporter
from aws.core.exporters.codedeploy.deployment.models import PaginatedCodeDeployDeploymentRequest
...
async def resync_codedeploy_deployment(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    service = ResyncAWSService(
        kind, CodeDeployDeploymentExporter, PaginatedCodeDeployDeploymentRequest, regional=True
    )
    async for batch in service:
        yield batch
