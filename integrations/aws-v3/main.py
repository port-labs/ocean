...
from aws.core.exporters.codebuild.build.models import PaginatedCodeBuildBuildRequest
...
@ocean.on_resync(ObjectKind.CODEBUILD_BUILD)
async def resync_codebuild_build(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    service = ResyncAWSService(
        kind, CodeBuildBuildExporter, PaginatedCodeBuildBuildRequest, regional=True
    )
    async for batch in service:
        yield batch
