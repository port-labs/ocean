...
from aws.core.exporters.codebuild.project_build_run.models import PaginatedProjectBuildRunRequest
...
async def resync_codebuild_project_build_run(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    service = ResyncAWSService(
        kind, CodeBuildProjectBuildRunExporter, PaginatedProjectBuildRunRequest, regional=True
    )
    async for batch in service:
        yield batch
