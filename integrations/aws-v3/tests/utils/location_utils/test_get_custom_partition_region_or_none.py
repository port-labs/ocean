...
class TestGetCustomPartitionRegionOrNone:
    @pytest.mark.asyncio
    async def test_returns_none_for_default_partition(
        self
    ) -> None:
        # Arrange
    ...
    @pytest.mark.asyncio
    async def test_returns_first_region_for_non_default_partition(
        self
    ) -> None:
        # Arrange
        mock_session = AsyncMock()
        with patch.object(
            LocationUtils, "get_partition", return_value="aws-us-gov"
        ), patch.object(
                LocationUtils,
                "get_first_available_region",
                new_callable=AsyncMock,
