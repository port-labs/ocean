...
class TestResyncStrategyFactory:
    ...
    @pytest.mark.asyncio
    async def test_create_single_account_performs_healthcheck(self, mock_single_account_config):
    ...
    @pytest.mark.asyncio
    async def test_create_single_account_healthcheck_failure(self, mock_single_account_config):
        """Test create handles healthcheck failure for single account."""
        with patch("aws.auth.session_factory.ocean") as mock_ocean:
            mock_ocean.integration_config = mock_single_account_config
            
            with patch.object(SingleAccountStrategy, "healthcheck", side_effect=Exception("Health check failed")):
...
class TestGetAllAccountSessions:
    ...
    @pytest.mark.asyncio
    async def test_get_all_account_sessions_single_account(self, mock_single_account_config, mock_aiosession):
        """Test get_all_account_sessions with single account strategy."""
        with patch("aws.auth.session_factory.ResyncStrategyFactory.create") as mock_create:
            mock_strategy = MagicMock(spec=SingleAccountStrategy)
            mock_strategy.get_account_sessions.return_value.__aiter__.return_value = [
                ({"Id": "123456789012", "Name": "Account 123456789012"}, mock_aiosession)
            ]
    ...
    @pytest.mark.asyncio
    async def test_get_all_account_sessions_multi_account(self, mock_multi_account_config, mock_aiosession):
        """Test get_all_account_sessions with multi account strategy."""
        with patch("aws.auth.session_factory.ResyncStrategyFactory.create") as mock_create:
            mock_strategy = MagicMock(spec=MultiAccountStrategy)
            mock_strategy.get_account_sessions.return_value.__aiter__.return_value = [
                ({"Id": "123456789012", "Name": "Account 123456789012"}, mock_aiosession),
                ({"Id": "987654321098", "Name": "Account 987654321098"}, mock_aiosession),
            ]
            mock_create.return_value = mock_strategy
    ...
    @pytest.mark.asyncio
    async def test_get_all_account_sessions_strategy_error(self, mock_single_account_config):
        """Test get_all_account_sessions handles strategy creation error."""
        with patch("aws.auth.session_factory.ResyncStrategyFactory.create", side_effect=Exception("Strategy error")):
            with pytest.raises(Exception, match="Strategy error"):
                async for _ in get_all_account_sessions():
                    pass

    @pytest.mark.asyncio
    async def test_get_all_account_sessions_yields_correct_types(self, mock_single_account_config, mock_aiosession):
