...
class TestResourceInspectorAlignment:
    ...
    async def test_action_id_key_merges_correctly_when_all_exist(self, mock_logger: MagicMock) -> None:
        # Arrange
        resource_type = _FakeResource().Type
        action_a = _make_action_class("ActionA", [{"a": 1, 'id': 1}, {"a": 2, 'id': 2}, {"a": 3, 'id': 3}])
        action_b = _make_action_class("ActionB", [{"b": 1, 'id': 1}, {'b': 2, 'id': 2}, {"b": 3, 'id': 3}])
    ...
    @pytest.mark.asyncio
    @patch(f'{patch_prefix}.logger')
    async def test_action_id_key_merges_correctly_when_not_all_exist(self, mock_logger: MagicMock) -> None:
        # Arrange
        resource_type = _FakeResource().Type
        action_a = _make_action_class("ActionA", [{"a": 1, 'id': 1}, {"a": 2, 'id': 2}, {"a": 3, 'id': 3}])
        action_b = _make_action_class("ActionB", [{"b": 3, 'id': 3}, {"b": 1, 'id': 1}, {'some': 'thing'}])
        ...
        # Assert
        mock_logger.warning.assert_called_once_with("Missing key 'id' in result: {'some': 'thing'}, skipping ...")
        assert resources == [
            {"Properties": {"a": 1, "b": 1, "id": 1}, 'Type': resource_type},
