from aws.core.exporters.s3.builder import S3BucketBuilder
from aws.core.exporters.s3.actions import (
...
class S3BucketInspector:
    ...
    async def inspect(self, bucket_name: str) -> S3Bucket:
        builder = S3BucketBuilder(bucket_name)
        results = await asyncio.gather(
            *(self._run_action(action, bucket_name) for action in self.actions)
        )
