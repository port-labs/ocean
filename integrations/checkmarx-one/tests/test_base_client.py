...
class TestBaseCheckmarxClient:
    ...
    def client(
        ...
        mock_rate_limiter: AsyncLimiter,
    ...
    def test_init_with_default_rate_limiter(
        self, client_with_default_rate_limiter: BaseCheckmarxClient
    ) -> None:
    ...
    async def test_auth_headers_property(
        self, client: BaseCheckmarxClient, mock_authenticator: AsyncMock
    ) -> None:
    ...
    async def test_send_api_request_with_params(
        self, client: BaseCheckmarxClient
    ) -> None:
        ...
        with patch.object(
            ...
            _ = await client._send_api_request("/test-endpoint", params=params)
    ...
    async def test_send_api_request_with_json_data(
        ...
        with patch.object(
            ...
            _ = await client._send_api_request(
    ...
    async def test_send_api_request_403_error(
        ...
        with patch.object(client.http_client, "request", return_value=mock_response):
            with pytest.raises(
                CheckmarxAPIError, match="Access denied. Please check your permissions."
            ):
                await client._send_api_request("/test-endpoint")
    ...
    @pytest.mark.asyncio
    async def test_send_api_request_429_error(
        self, client: BaseCheckmarxClient
    ) -> None:
    ...
    async def test_send_api_request_other_http_error(
        ...
        with patch.object(client.http_client, "request", return_value=mock_response):
            with pytest.raises(
                CheckmarxAPIError, match="API request failed: Internal Server Error"
            ):
                await client._send_api_request("/test-endpoint")
    ...
    async def test_send_api_request_unexpected_error(
        ...
        with patch.object(
            client.http_client, "request", side_effect=Exception("Network error")
        ):
            with pytest.raises(
                CheckmarxAPIError, match="Request failed: Network error"
            ):
                await client._send_api_request("/test-endpoint")
    ...
    async def test_get_paginated_resources_multiple_pages(
        ...
        with patch.object(
            client, "_send_api_request", side_effect=[first_response, second_response]
        ):
            results = []
            async for batch in client._get_paginated_resources("/test", "data"):
                results.extend(batch)
    ...
    async def test_get_paginated_resources_with_params(
        ...
        with patch.object(
            client, "_send_api_request", return_value=mock_response
        ) as mock_request:
            params = {"status": "active"}
            results = []
            async for batch in client._get_paginated_resources(
    ...
    async def test_get_paginated_resources_request_error(
        ...
        with patch.object(
            client, "_send_api_request", side_effect=Exception("Request failed")
        ):
            results = []
            async for batch in client._get_paginated_resources("/test", "data"):
                results.extend(batch)
