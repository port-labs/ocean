...
class TestInitializeClient:
    ...
    def test_init_client_missing_base_url(self, mock_ocean):
        ...
        # Act & Assert
        with pytest.raises(CheckmarxAuthenticationError, match="checkmarx_base_url, checkmarx_iam_url, and checkmarx_tenant are required"):
            init_client()

    @patch('initialize_client.ocean')
    def test_init_client_missing_iam_url(self, mock_ocean):
    ...
    @patch('initialize_client.ocean')
    def test_init_client_missing_tenant(self, mock_ocean):
    ...
    @patch('initialize_client.ocean')
    def test_init_client_missing_all_required_config(self, mock_ocean):
    ...
    @patch('initialize_client.ocean')
    def test_init_client_missing_auth_credentials(self, mock_ocean):
    ...
    @patch('initialize_client.ocean')
    def test_init_client_incomplete_oauth_credentials(self, mock_ocean):
    ...
    @patch('initialize_client.ocean')
    def test_init_client_logs_error_on_auth_failure(self, mock_ocean, mock_logger):
    ...
    @patch('initialize_client.logger')
    @patch('initialize_client.ocean')
    def test_init_client_logs_error_on_client_creation_failure(self, mock_ocean, mock_logger, mock_client_class):
        ...
        # Act & Assert
        with pytest.raises(CheckmarxAuthenticationError, match="Client initialization failed"):
            init_client()

        mock_logger.error.assert_called_with("Failed to initialize Checkmarx One client: Client creation failed")
