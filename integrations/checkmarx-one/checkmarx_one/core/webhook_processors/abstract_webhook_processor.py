...
class CheckmarxOneAbstractWebhookProcessor(AbstractWebhookProcessor):
    ...
    async def _verify_webhook_signature(self, request: Request) -> bool:
        ...
        signature = request.headers.get("x-checkmarx-signature")
        if not signature:
            logger.error(
                "Missing 'x-checkmarx-signature' header. Webhook authentication failed."
            )
            return False
