...
- Optionally configure `webhookSigningSecret` to sign outgoing v0 webhooks and verify incoming callbacks. When unset, signature verification is skipped. When set, that value is sent to Cursor as the webhook secret and used to verify `X-Webhook-Signature`. The callback URL is static (`/integration/webhook`).
