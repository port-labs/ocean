...
#         if group:
#             await ocean.register_raw(ObjectKind.GROUP, [self.gitlab_service.enrich_group_with_members(group)])
#         elif (
#             group_full_path
...
class Groups(GroupHandler):
    events = ['Subgroup Hook', "Member Hook"]
    system_events = [
        "group_destroy",
        "group_create",
        "group_rename",
    ...
    async def _on_hook(self, body: dict[str, Any], gitlab_group: Group) -> None: 
        print("Event Triggered in group handler", body)
        if gitlab_group and self.gitlab_service.should_run_for_group(gitlab_group):
            await ocean.register_raw(ObjectKind.GROUP, [await self.gitlab_service.enrich_group_with_members(gitlab_group)])
