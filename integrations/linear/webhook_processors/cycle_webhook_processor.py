...
class CycleWebhookProcessor(_LinearAbstractWebhookProcessor):
    """Processes cycle-related webhook events from Linear."""

    async def should_process_event(self, event: WebhookEvent) -> bool:
        return event.headers.get(
            "linear-event"
        ) == "Cycle" and await self.is_action_allowed(event.payload.get("action", ""))

    async def get_matching_kinds(self, event: WebhookEvent) -> list[str]:
        return [ObjectKind.CYCLE]

    async def handle_event(
        self, payload: EventPayload, resource_config: ResourceConfig
    ) -> WebhookEventRawResults:
        event_data = payload["data"]
        cycle_id = event_data["id"]
        action = payload["action"]

        logger.info(f"Processing webhook event for cycle with ID: {cycle_id}")
