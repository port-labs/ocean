...
from github.utils import ObjectKind, PullRequestState
...
class TestPullRequestWebhookProcessor:
    ...
    async def test_validate_payload_valid(
        ...
        for action in PULL_REQUEST_EVENTS:
            ...
            assert (
                await pull_request_webhook_processor.validate_payload(payload) is True
            )
    ...
    async def test_validate_payload_invalid(
        ...
        # Missing action
        payload = {"pull_request": {"number": 101}, "repository": {"name": "test-repo"}}
    ...
    @pytest.mark.parametrize(
        ...
        [
            (PullRequestState.OPEN, "opened", True, False),
            (PullRequestState.OPEN, "closed", False, True),
            (PullRequestState.CLOSED, "opened", True, False),
            (PullRequestState.CLOSED, "closed", True, False),
            (PullRequestState.ALL, "opened", True, False),
            (PullRequestState.ALL, "closed", True, False),
        ],
    )
    async def test_handle_event_with_selector_state(
        self,
        selector_state: PullRequestState,
        action: str,
        ...
        with patch(
            "github.webhook.webhook_processors.pull_request_webhook_processor.PullRequestExporter",
