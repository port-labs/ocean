...
class PullRequestWebhookProcessor(_GithubAbstractWebhookProcessor):
    ...
    async def handle_event(
        ...
        if action == "closed" and config.selector.state == PullRequestState.OPEN:
            logger.info(f"Pull request {repo_name}/{number} was closed")
        ...
        exporter = PullRequestExporter(create_github_client())
    ...
    async def validate_payload(self, payload: EventPayload) -> bool:
        ...
        return bool(
            payload["repository"].get("name") and payload["pull_request"].get("number")
        )
