...
class FirehydrantClient:
    def __init__(
        self, base_url: str, api_key: str, app_host: str
    ):
        """
    ...
    async def send_paginated_api_request(
        ...
        try:
            ...
            while True:
                ...
                if paging_info and paging_info.get("page", 0) < paging_info.get("pages", 0):
    ...
    async def get_paginated_resource(self, resource_type: str) -> AsyncGenerator[list[dict[str, Any]], None]:
        ...
        try:
            while True:
                response = await self.send_api_request(endpoint=endpoint, query_params=pagination_params)
                ...
                if pagination_params["page"] > response.get("pagination", {}).get("pages"):
                    break

        except Exception as e:
            logger.error(f"Error while fetching {resource_type}: {str(e)}")

    async def enrich_incident_report_data(self, report: dict[str, Any]) -> dict[str, Any]:
        """Enriches the incident data"""

        ## get incident task completion summary statistics
        incident_id = report.get("incident", {}).get("id")
        task_completion_status = await self.get_tasks_by_incident(incident_id = incident_id)
        ...
        retro_responses = await self.get_retrospective_questions_and_answers(questions = report.get("questions", []))
    ...
    async def get_tasks_by_incident(self, incident_id: str) -> dict[str, Any]:
        ...
        tasks = await self.send_paginated_api_request(endpoint=task_url, query_params=params, data_key="data")
        task_completion_status = await self.get_task_completion_status(tasks)
        return task_completion_status
    
    async def get_task_completion_status(self, tasks: list[dict[str, Any]]) -> dict[str, Any]:
        """Returns statistics about completed and uncompleted tasks"""
        ...
        return {"completed": total_completed_tasks, "incompleted": len(tasks) - total_completed_tasks}

    async def compute_incident_duration(self, milestones: list[dict[str, Any]]) -> float:
        """Computes incident duration"""
        started_time = None
        resolved_time = None
    ...
    async def get_retrospective_questions_and_answers(self, questions: list[dict[str, Any]]) -> float:
        """Format retrospective questions and answers"""
        
        retro_responses_list = []
        for question in questions:
            retro_responses_list.append({"question": question.get("title"), "answer": question.get("body")})
        return retro_responses_list


    async def compute_service_mean_time_metrics(self, active_incidents_ids: list[dict[str, Any]]) -> dict[str, Any]:
        """Computes mean time service details"""
        ...
        for incident_id in active_incidents_ids:
            ...
            incident_metrics = await self.compute_incident_analytics_metrics(milestones = milestones)
            logger.warning(f"metrics {incident_metrics}")
        ...
        logger.error(service_metrics)
        return service_metrics

    async def compute_incident_analytics_metrics(self, milestones: list[dict[str, Any]]) -> dict[str, Any]:
        incident_times = {"started": None, "acknowledged": None, "mitigated": None, "identified": None, "resolved": None}
