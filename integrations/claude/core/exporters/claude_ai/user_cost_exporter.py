...
class ClaudeAIUserCostExporter(AbstractClaudeExporter):
    """Per-user USD cost across a date range (Claude AI / Enterprise).
    ...
    async def get_paginated_resources(
        ...
        async for batch in self.client.send_paginated_request(
            ...
            logger.debug(f"Fetched user cost batch with {len(batch)} records")
            yield batch
