...
async def test_generate_files_does_not_stop_after_empty_project() -> None:
    """Test that generate_files continues processing after a project with no repositories.

    Regression test for PORT-17439: File kind stops after empty project.
    When generate_repositories yields an empty batch (a project with no repos),
    generate_files should skip it and continue to the next batch rather than stopping.
    """
    paths = ["**/*.md"]
    ...
    # Yield empty list first (empty project), then a project with one repo
    async def mock_generate_repositories(
        include_disabled_repositories: bool = True,
    ) -> AsyncGenerator[list[dict[str, Any]], None]:
        yield []  # empty project — should NOT stop processing
        yield [mock_repo]
