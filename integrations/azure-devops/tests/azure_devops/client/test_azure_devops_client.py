...
async def test_generate_work_items_raises_on_malformed_batch(
    ...
    """
    Tests that a JSONDecodeError on a work item batch re-raises after logging,
    aborting the kind resync so the framework skips the delete phase.
    Skipping the batch and continuing would cause incorrect deletes of the ~200
    work items in the truncated batch that already exist in Port.
    """
    import json
        ...
        if "wit/workitems" in url:
            ...
            if call_count == 2:
                return Response(
                    status_code=200,
                    request=Request(method, url),
                    content=b'{"value": [{"id": 201, "fields": {"System.Title": "Item 201"',  # truncated
            ...
            return Response(
                status_code=200,
                request=Request(method, url),
                json={"value": [{"id": i, "fields": {}} for i in ids]},
            )
    ...
    async with event_context("test_event"):
        with (
            ...
            # Batch 1 succeeded before the error; batch 2 aborted; batch 3 never ran
