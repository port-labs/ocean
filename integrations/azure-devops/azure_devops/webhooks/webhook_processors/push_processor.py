...
class PushWebhookProcessor(_AzureDevOpsBaseWebhookProcessor):
    ...
    async def handle_event(
        ...
        repository = await self._get_repository_data(push_data)
        if repository:
            processed_entities.append(repository)
    ...
    async def _sync_changed_files(
        ...
        try:
            repo_info = await self.client.get_repository(repo_id)
            if not repo_info:
    ...
    async def _build_file_entity_from_commit(
        ...
        try:
            ...
            file_content = await self.client.get_file_by_commit(
    ...
    async def _get_repository_data(
        self, push_data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        repository_id = push_data["repository"]["id"]
        repository = await self.client.get_repository(repository_id)
        if not repository:
