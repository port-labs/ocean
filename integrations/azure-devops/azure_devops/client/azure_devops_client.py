...
# Maximum subject descriptors per single Identities API request.
MAX_IDENTITIES_PER_REQUEST = 100
...
class AzureDevopsClient(HTTPBaseClient):
    ...
    # ── Identities-path helpers (no PCA required) ─────────────────────────────

    async def _enumerate_graph_users(
        self,
    ) -> AsyncGenerator[list[dict[str, Any]], None]:
        """Page through all org members via the Graph Users API.
    ...
    async def _fetch_identities_for_descriptors(
        self,
        subject_descriptors: list[str],
        query_membership: str = "None",
    ) -> list[dict[str, Any]]:
        """Batch-fetch Identity objects for the given Graph subject descriptors.

        Splits the list into MAX_IDENTITIES_PER_REQUEST-sized chunks so we stay
        within the ADO API limit.  Set query_membership='expandedUp' to ask ADO
        to populate each identity's memberOf field with the full upward group
        chain — this is what lets us derive project membership without PCA.
        """
        results: list[dict[str, Any]] = []
        total = len(subject_descriptors)
        for batch_num, batch in enumerate(
            batched(subject_descriptors, MAX_IDENTITIES_PER_REQUEST), start=1
        ):
            params: dict[str, Any] = {
                "subjectDescriptors": ",".join(batch),
            ...
            if not response:
                logger.warning(
                    f"No response for identities batch {batch_num} "
                    f"({len(batch)} of {total} descriptor(s))"
                )
                continue
    ...
    @staticmethod
    def _is_builtin_identity(identity: dict[str, Any]) -> bool:
        """Return True for system accounts that must never appear as Port entities.

        Azure DevOps populates every org with built-in identities:
        - '[TEAM FOUNDATION]\\...' — collection-scoped service accounts, build agents.
        - Descriptors starting with 'scp.' — project/org scope entries, not real users.
        """
        if identity.get("providerDisplayName", "").startswith("[TEAM FOUNDATION]"):
            return True
        return identity.get("subjectDescriptor", "").startswith("scp.")

    async def _build_project_scope_map(self) -> dict[str, dict[str, Any]]:
        """Return a {project_id: project} mapping built from generate_projects().

        generate_projects() is cached for the lifetime of the event, so this
        adds no extra API calls when the resync is already fetching projects.
        The map is keyed by the project GUID which matches the scopeId field
        on project-scoped group identities returned by the Identities API.
        """
        project_map: dict[str, dict[str, Any]] = {}
        async for batch in self.generate_projects():
            for project in batch:
                project_map[project["id"]] = project
        return project_map

    @staticmethod
    def _extract_project_entitlements(
        member_of_identities: list[dict[str, Any]],
        projects_by_id: dict[str, dict[str, Any]],
    ) -> list[dict[str, Any]]:
        """Convert a user's group-membership chain into projectEntitlements entries.

        Each group identity that has a scopeId matching a known project contributes
        one entry.  Duplicates (user is in multiple groups within the same project)
        are deduplicated so each project appears at most once.

        The returned shape is a minimal subset of the projectEntitlements object
        from the User Entitlements API: just projectRef (id + name) and the group
        display name.  Mappings that only access projectRef.id / projectRef.name
        will work unchanged; mappings that reference deeper fields (teamRefs,
        joinedDate, …) will see null and should be updated accordingly.
        """
        seen_project_ids: set[str] = set()
        entitlements: list[dict[str, Any]] = []
        for group_identity in member_of_identities:
            scope_id = group_identity.get("scopeId", "")
            if not scope_id or scope_id in seen_project_ids:
                continue
            project = projects_by_id.get(scope_id)
            if not project:
                continue
            seen_project_ids.add(scope_id)
            entitlements.append(
                {
                    "projectRef": {
                        "id": project["id"],
                    ...
                    "group": {
                        "displayName": group_identity.get("providerDisplayName", ""),
                    },
                }
            )
        return entitlements
    ...
    @staticmethod
    def _build_user_shape_from_graph_user(
        graph_user: dict[str, Any],
        project_entitlements: list[dict[str, Any]],
    ) -> dict[str, Any]:
        """Wrap a Graph user in the same envelope as a User Entitlement object.

        Produces a dict whose structure mirrors the userentitlements API response
        so that existing Port JQ mappings — identifier: .user.principalName,
        title: .user.displayName, email: .user.mailAddress — work without change.

        accessLevel fields are set to None because license information is only
        available via the Member Entitlement Management API (which requires PCA).
        Mappings that reference .accessLevel.accountLicenseType should guard with
        .accessLevel?.accountLicenseType // "unknown".
        """
        return {
            "user": {
                "subjectKind": graph_user.get("subjectKind", "user"),
                "displayName": graph_user.get("displayName"),
                "mailAddress": graph_user.get("mailAddress"),
                "principalName": graph_user.get("principalName"),
                "descriptor": graph_user.get("descriptor"),
                "url": graph_user.get("url"),
                "origin": graph_user.get("origin"),
                "originId": graph_user.get("originId"),
                "domain": graph_user.get("domain"),
                "_links": graph_user.get("_links"),
            },
            "id": graph_user.get("originId") or graph_user.get("descriptor"),
            "accessLevel": {
                "accountLicenseType": None,
                "licenseDisplayName": None,
                "status": None,
            },
    ...
    async def generate_users_via_identities(
        self,
    ) -> AsyncGenerator[list[dict[str, Any]], None]:
        """Enumerate users + project membership via the Graph and Identities APIs.

        Requires only vso.identity (Identity-read) — no Project Collection
        Administrator (PCA) needed.  Yields dicts shaped identically to
        generate_users() so existing Port mappings work unchanged.

        Algorithm per batch of Graph users:
        1. Fetch their full Identity objects with queryMembership=expandedUp so
           each identity's memberOf field contains all group memberships.
        2. Collect the unique group descriptors from every memberOf chain.
        3. Batch-resolve those group identities to retrieve their scopeId, which
           is the project GUID for project-scoped groups.
        4. Filter out built-in system identities ([TEAM FOUNDATION]\\... or scp.*).
        5. Map each user's group memberships to projectEntitlements entries using
           the pre-built project-scope map.
        """
        projects_by_id = await self._build_project_scope_map()

        async for graph_user_batch in self._enumerate_graph_users():
            descriptors = [
                u["descriptor"] for u in graph_user_batch if u.get("descriptor")
            ]
            if not descriptors:
                continue

            identities = await self._fetch_identities_for_descriptors(
                descriptors, query_membership="expandedUp"
            )
            identity_by_descriptor = {
                i["subjectDescriptor"]: i
                for i in identities
                if i.get("subjectDescriptor")
            }

            # Collect all group descriptors from memberOf chains across this batch,
            # then resolve them in one batched call to get their scopeId values.
            all_group_descriptors: set[str] = set()
            for identity in identities:
                for group_ref in identity.get("memberOf", []):
                    ...
                    if desc:
                        all_group_descriptors.add(desc)

            group_identities_by_descriptor: dict[str, dict[str, Any]] = {}
            if all_group_descriptors:
                group_identity_list = await self._fetch_identities_for_descriptors(
                    list(all_group_descriptors)
                )
                group_identities_by_descriptor = {
                    g["subjectDescriptor"]: g
                    for g in group_identity_list
                    if g.get("subjectDescriptor")
                }
            ...
            for graph_user in graph_user_batch:
                ...
                if identity and self._is_builtin_identity(identity):
                    logger.debug(
                        f"Skipping built-in identity: "
                        f"{identity.get('providerDisplayName')}"
                    )
                    continue
                ...
                shaped_batch.append(
                    self._build_user_shape_from_graph_user(
                        graph_user, project_entitlements
                    )
                )
