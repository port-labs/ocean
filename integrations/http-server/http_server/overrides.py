...
class ApiPathParameter(BaseModel):
    ...
    query_params: Optional[Dict[str, Any]] = Field(default=None, description="Query parameters for discovery endpoint")
    headers: Optional[Dict[str, str]] = Field(default=None, description="Headers for discovery endpoint")
...
class HttpServerSelector(Selector):
    ...
    endpoint: Optional[str] = Field(default=None, description="HTTP endpoint path (supports {param} templates)")
    method: str = Field(default="GET", description="HTTP method")
    query_params: Optional[Dict[str, Any]] = Field(default=None, description="Query parameters")
    headers: Optional[Dict[str, str]] = Field(default=None, description="HTTP headers")
    path_parameters: Optional[Dict[str, ApiPathParameter]] = Field(default=None, description="Dynamic path parameters")
    data_path: Optional[str] = Field(default=None, description="JQ path to extract data array from response (e.g., '.members', '.data.items')")
...
class HttpServerPortAppConfig(PortAppConfig):
    ...
    resources: list[HttpServerResourceConfig] = Field(default_factory=list)
