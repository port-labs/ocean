...
class HttpServerClient:
    ...
    async def fetch_paginated_data(
        ...
        async def _fetch():
            async for batch in self._fetch_with_pagination(endpoint, method, query_params, headers):
                yield batch
    ...
    async def _fetch_with_pagination(
        ...
        logger.info(f"Fetching data from {method} {url} with pagination: {pagination_type}")
        
        if pagination_type == "none":
            # Single request, no pagination
            async for batch in self._fetch_single_page(url, method, params, request_headers):
                yield batch
        
        elif pagination_type == "offset":
            async for batch in self._fetch_offset_paginated(url, method, params, request_headers):
                yield batch
        
        elif pagination_type == "page":
            async for batch in self._fetch_page_paginated(url, method, params, request_headers):
                yield batch
        
        elif pagination_type == "cursor":
            async for batch in self._fetch_cursor_paginated(url, method, params, request_headers):
                yield batch
    ...
    async def fetch_multiple_endpoints(
        ...
        tasks = [
            semaphore_async_iterator(
                self.semaphore,
                functools.partial(self.fetch_paginated_data, endpoint, method, query_params, headers)
            )
