...
from http_server.overrides import HttpServerResourceConfig, HttpServerSelector, ApiPathParameter
...
async def query_port_entities(param_config: ApiPathParameter) -> List[str]:
    ...
    try:
        ...
        if values:
            logger.info(f"Extracted parameter values: {values[:5]}{'...' if len(values) > 5 else ''}")
        else:
