...
class SonarQubeComponentSearchFilter(BaseModel):
    query: str | None = Field(description="Limit search to component names that contain the supplied string")
    metrics: list[dict[str, str]] | None = Field(description="List of metric keys and their values such as security_rating>=2 or coverage<=80")
    alert_status: str| None = Field(alias="alertStatus", description="To filter on a Quality Gate status")
    languages: Union[str, list[str]] | None = Field(description="List of languages or a single language")
    tags: Union[str, list[str]] | None = Field(description="List of tags or a single tag")
    qualifier: Literal["TRK", "APP"] | None = Field(description="To filter on a component qualifier")
...
class SonarQubeProjectAPIQueryParams(BaseModel):
    s: str | None = Field(description="Sort projects by numeric metric key such as alert_status, analysisDate, new_coverage etc")
...
class SonarQubeIssueAPIQueryParams(BaseModel):
    assigned: Literal["yes", "no", "true", "false"] | None = Field(description="To retrieve assigned or unassigned issues")
    ...
    clean_code_attribute_categories: (
        ...
        | None
    ) = Field(alias="cleanCodeAttributeCategories", description="List of clean code attribute categories")
    code_variants: list[str] | None = Field(alias="codeVariants", description="List of code variants")
    created_before: str | None = Field(alias="createdBefore", description="To retrieve issues created before the given date")
    created_after: str | None = Field(alias="createdAfter", description="To retrieve issues created after the given date")
    ...
    ) = Field(alias="impactSoftwareQualities", description="List of impact software qualities")
    statuses: (
    ...
    owasp_asvs_level: Literal["1", "2", "3"] | None = Field(alias="owaspAsvsLevel", description="OWASP ASVS level")
    resolved: Literal["yes", "no", "true", "false"] | None = Field(description="To retrieve resolved or unresolved issues")
    ...
    sonarsource_security: list[str] | None = Field(alias="sonarsourceSecurity", description="List of SonarSource security categories")
