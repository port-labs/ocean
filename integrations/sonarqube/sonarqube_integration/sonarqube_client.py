...
class SonarQubeClient:
    def __init__(self, base_url: str, api_key: str, organization_id: str, app_host: str, http_client: httpx.AsyncClient):
        """
    ...
    async def send_api_request(self, endpoint: str, method: str = 'GET', query_params: Optional[dict[str, Any]] = None, json_data: Optional[dict[str, Any]] = None) -> dict[str, Any]:
        """
    ...
    async def get_projects(self) -> list[Any]:
        ...
        response = await self.send_api_request(endpoint=endpoint_url, query_params={"organization": self.organization_id})
    ...
    async def get_branches(self, project_key: str) -> Optional[dict[str, Any]]:
        ...
        response = await self.send_api_request(endpoint=endpoint_url, query_params={"project": project_key})
    ...
    async def get_quality_gate_status(self, project_key: str) -> Optional[dict[str, Any]]:
        ...
        response = await self.send_api_request(endpoint=endpoint_url, query_params={"projectKey": project_key})
    ...
    async def get_quality_gate_name(self, project_key: str) -> Optional[str]:
        ...
        response = await self.send_api_request(endpoint=endpoint_url, query_params={"project": project_key, "organization": self.organization_id})
    ...
    async def get_cloud_analysis_data_for_project(self, project: dict[str, Any]) -> dict[str, Any]:
        """
        ...
        """
        logger.info(f'Fetching sonarqube quality analysis data for project: {project.get("key")}')
        ...
        project_quality_status = await self.get_quality_gate_status(project_key=project_key)
    ...
    async def get_sonarqube_cloud_analysis(self) -> list[dict[str, Any]]:
        ...
        """
        logger.info(f'Fetching all quality analysis data in organization: {self.organization_id}')
    ...
    async def get_or_create_webhook_url(self) -> None:
        ...
        for project in projects:
            ...
            webhooks_response = await self.send_api_request(endpoint=f'{webhook_endpoint}/list', query_params={
                "project": project_key,
        ...
        for webhook in webhooks_to_create:
            await self.send_api_request(endpoint=f'{webhook_endpoint}/create', method='POST', query_params={
                **webhook,
                "url": invoke_url
            })
