...
def limiter() -> Generator[RollingWindowLimiter[None], None, None]:
    ...
    # Clear the queue to prevent hanging
    try:
...
@pytest.mark.asyncio
async def test_custom_logger() -> None:
    """Test custom logger functionality."""
    # Create a proper logger with a StringIO handler
    ...
    # Add a real handler with a level
    ...
    # Create the limiter with our logger
    limiter: RollingWindowLimiter[None] = RollingWindowLimiter[None](
        limit=1, window=0.5, logger=logger
    )

    try:
        # Verify logger was set
        assert limiter._logger is logger

        # Operation should log
        await limiter.acquire()

        # Should have logged something
        log_output = log_stream.getvalue()
        assert log_output, "Logger should have output something"
        assert (
            "Created rate limiter" in log_output
            or "processed immediately" in log_output
        )
    finally:
        # Ensure cleanup
        limiter._shutdown_event.set()
