...
from typing import Optional, Dict, Any, Callable, TypeVar, Coroutine, Generic, List, Deque
...
class RollingWindowLimiter(AbstractAsyncContextManager):
    ...
    asyncio.Queue for managing pending requests. It efficiently schedules processing
    ...
    """
    __slots__ = (
        "limit", "window", "_timestamps", "_lock", "_condition", 
        "_queue", "_processing", "_next_wake_task", "_logger",
        "_metrics", "_timestamp_last_purged", "_purge_interval",
        "_request_timeout", "_task_timeout", "_shutdown_event"
    )
    ...
    def __init__(
        ...
        Args:
            ...
            logger: Custom logger instance to use (None = create a new one).
        ...
        # Logging and metrics
        self._logger = logger or logging.getLogger(f"{__name__}.RollingWindowLimiter")
        self._metrics = {
            "total_requests": 0,
            "rejected_requests": 0,
            "timeouts": 0,
        ...
        if len(self._timestamps) < self.limit:
            if not self._queue.empty():
                ...
                if not future.done():
                    ...
                    self._logger.warning("Future was already done (cancelled or timed out)")
    ...
    async def _calculate_next_slot_time(self) -> float:
        ...
        # Adde a tiny minimum to prevent CPU spinning, but keeping it very small
        ...
        try:
            while not self._queue.empty() and not self._shutdown_event.is_set():
                ...
                if not processed and not self._queue.empty():
                    # Calculate the *exact* time when the next slot will open up
                    ...
                    self._logger.debug(f"Will process next request in {wait_time:.6f}s when oldest timestamp expires")
                    
                    try:
                        await self._safe_cancel_task(self._next_wake_task)
                        
                        self._next_wake_task = asyncio.create_task(asyncio.sleep(wait_time))
                        
                        await self._next_wake_task
                    except asyncio.CancelledError:
                        self._logger.debug("Wake-up task was cancelled")
                    finally:
    ...
    async def acquire(self, timeout: Optional[float] = None) -> bool:
        ...
        async with self._lock:
            ...
            if purged_count > 0:
                self._logger.debug(f"Purged {purged_count} expired timestamps during acquire")
            ...
            self._metrics["max_queue_size"] = max(self._metrics["max_queue_size"], queue_size)
        ...
        try:
            ...
            self._metrics["errors"] += 1
            self._logger.error(f"Error while waiting for rate limit: {e}", exc_info=True)
            raise
    ...
    def limit_function(self, func: Callable[..., Coroutine[Any, Any, R]]) -> Callable[..., Coroutine[Any, Any, R]]:
        """
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if metrics["total_requests"] > 0:
            metrics["rejection_rate"] = metrics["rejected_requests"] / metrics["total_requests"]
        ...
        metrics["current_utilization"] = len(self._timestamps) / self.limit if self.limit > 0 else 0
