from typing import Any, AsyncGenerator, Optional, TypeVar, Generic, Dict, Tuple, Set
import asyncio
import time
...
class BaseRotatingClient:
    ...
    def __init__(self, base_client: BitbucketBaseClient):
        ...
        self.base_client_queue = asyncio.Queue()
    ...
    def add_alternative_base_client(self, base_client: BitbucketBaseClient, limiter: Optional[RollingWindowLimiter] = None) -> None:
        """
        ...
        if base_client not in self.client_packages:
            ...
            logger.info(f"Added alternative base client to rotation queue")
        else:
            logger.info(f"Base client already in rotation queue")
    ...
    async def _rotate_base_client(self) -> None:
        ...
        self.current_limiter = self.client_packages.get(next_base_client)
        logger.info(f"Rotated to alternative base client with rate limiter: {self.current_limiter is not None}")
    ...
    def get_earliest_available_client(self) -> Tuple[Optional[BitbucketBaseClient], float]:
        """
        ...
        for base_client, limiter in self.client_packages.items():
            if limiter and limiter.has_capacity():
                ...
                next_available = limiter.next_available_time() if limiter else float('inf')
    ...
    async def _ensure_client_available(self) -> None:
        ...
        if self.current_limiter and not self.current_limiter.has_capacity():
            ...
            if self.has_tried_all_clients():
                ...
                if earliest_client:
                    logger.info(f"All base clients have no capacity, queueing on first available client at {earliest_time}")
