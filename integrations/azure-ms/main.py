...
async def on_resync(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async with (
        ...
        resource_containers = ResourceContainers(azure_client)

        for subscriptions in subscriptions_batches:
            logger.info("Running full resource container sync")
            async for resource_containers in resource_containers.sync_full(
...
async def on_resync(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async with (
        ...
        resources = Resources(azure_client)

        for subscriptions in subscriptions_batches:
            logger.info("Running full resource sync")
            async for resources in resources.sync_full(
                [s.subscription_id for s in subscriptions],
