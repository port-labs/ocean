...
def make_run(execution_properties: dict) -> WorkflowNodeRun:
...
class TestTriggerPipelineExecutor:
    async def test_happy_path(
        ...
        with patch(
            "gitlab.actions.trigger_pipeline_executor.ocean"
        ) as mock_ocean:
            mock_ocean.port_client = mock_port_client
            await executor.execute(run)

        executor.client.trigger_pipeline.assert_called_once_with(
    ...
    async def test_report_pipeline_status_false_completes_immediately(
        ...
        run = make_run(
            {"project": "my-group/my-project", "ref": "main", "reportPipelineStatus": False}
        )
        with patch(
            "gitlab.actions.trigger_pipeline_executor.ocean"
        ) as mock_ocean:
    ...
    async def test_incomplete_response_raises(
        ...
        executor.client.trigger_pipeline = AsyncMock(return_value={"id": 99})
        run = make_run({"project": "my-group/my-project", "ref": "main"})
    ...
    async def test_pipeline_variables_null_treated_as_empty(
        ...
        run = make_run({"project": "my-group/my-project", "ref": "main", "pipelineVariables": None})
    ...
    async def test_pipeline_variables_non_dict_raises(
        ...
        run = make_run({"project": "my-group/my-project", "ref": "main", "pipelineVariables": ["a", "b"]})
        with pytest.raises(MissingExecutionPropertyError, match="pipelineVariables must be a key-value object"):
            await executor.execute(run)
    ...
    async def test_pipeline_variables_serialized(
        ...
        _, call_ref, call_vars = executor.client.trigger_pipeline.call_args[0]
