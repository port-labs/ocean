...
class TestTriggerPipelineExecutor:
    ...
    async def test_identity_run_uses_user_token_client(
        ...
        with (
            ...
            patch(
                "gitlab.actions.trigger_pipeline_executor.create_user_gitlab_client",
                return_value=user_client,
            ) as mock_create_user_client,
        ):
            mock_ocean.port_client = mock_port_client
            await executor.execute(run)

        mock_resolve_user_token.assert_awaited_once_with(run)
        mock_create_user_client.assert_called_once_with("user-oauth-token")
