...
class TestProjectWithMemberWebhookProcessor:
    ...
    async def test_validate_payload(
        ...
        # Group-hook member events omit project_id but carry project_path_with_namespace
        group_hook_member_payload = {
            "event_name": "user_add_to_team",
            "project_path_with_namespace": "group1/my-project",
            "user_username": "jsmith",
            "user_id": 41,
            "group_id": 7,
        }
        assert await processor.validate_payload(group_hook_member_payload) is True

        # Member event with neither project_id nor project_path_with_namespace is invalid
        member_payload_no_ref = {
            "event_name": "user_add_to_team",
            "user_username": "jsmith",
        }
        assert await processor.validate_payload(member_payload_no_ref) is False

        # Same for user_remove_from_team and user_update_for_team
        for evt in ("user_remove_from_team", "user_update_for_team"):
            assert (
            ...
            assert await processor.validate_payload({"event_name": evt}) is False

    async def test_handle_event_group_hook_member_add(
        self,
        processor: ProjectWithMemberWebhookProcessor,
    ) -> None:
        """Group-hook user_add_to_team payload has no project_id; handler resolves via path."""
        group_hook_payload = {
            "event_name": "user_add_to_team",
            "project_path_with_namespace": "group1/my-project",
            "user_username": "jsmith",
        ...
        expected_project = {
            "id": 74,
            "path_with_namespace": "group1/my-project",
            "__members": [{"id": 41, "username": "jsmith", "name": "John Smith"}],
        }

        processor._gitlab_webhook_client = MagicMock()
        processor._gitlab_webhook_client.get_project = AsyncMock(
            return_value={"id": 74, "path_with_namespace": "group1/my-project"}
        )
        processor._gitlab_webhook_client.enrich_project_with_members = AsyncMock(
            return_value=expected_project
        )

        result = await processor.handle_event(group_hook_payload, resource_config)

        # Must resolve project by path, not by numeric ID
        processor._gitlab_webhook_client.get_project.assert_called_once_with(
            "group1/my-project"
        )
        processor._gitlab_webhook_client.enrich_project_with_members.assert_called_once_with(
            {"id": 74, "path_with_namespace": "group1/my-project"},
            include_bot_members=False,
            include_inherited_members=True,
        )
        assert len(result.updated_raw_results) == 1
