...
from okta.webhook_processors.application_webhook_processor import ApplicationWebhookProcessor
...
class TestApplicationWebhookProcessor:
    """Test cases for ApplicationWebhookProcessor."""

    @pytest.fixture
    def processor(self):
        """Create an ApplicationWebhookProcessor instance."""
        with patch('okta.webhook_processors.application_webhook_processor.create_okta_client'):
            return ApplicationWebhookProcessor()

    @pytest.fixture
    def app_event(self):
        """Create an application webhook event."""
        return WebhookEvent(
            trace_id="test-trace-id",
            payload={
                "eventType": "app.lifecycle.create",
                "published": "2023-01-01T00:00:00Z",
                "eventId": "test-event-id",
                "target": [
                    {
    ...
    @pytest.mark.asyncio
    async def test_should_process_event_app_event(self, processor, app_event):
        """Test that application events are processed."""
        assert await processor._should_process_event(app_event) is True

    @pytest.mark.asyncio
    async def test_get_matching_kinds(self, processor, app_event):
        """Test that application events match the correct kind."""
        kinds = await processor.get_matching_kinds(app_event)
        assert kinds == ["okta-application"]
