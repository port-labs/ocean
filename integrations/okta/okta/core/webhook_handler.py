...
from .live_events import OktaLiveEventProcessor
...
class OktaWebhookHandler:
        ...
        for event_data in events:
            try:
                if processed_event := self.processor.process_event(event_data):
                    ...
                    logger.debug(f"Ignored event: {event_data.get('eventType', 'unknown')}")
    ...
    def extract_resource_updates(
        ...
        for event in processed_events:
            if not (resource_type := event.get("resource_type")) or not (resource_data := event.get("resource_data", {})):
                continue
    ...
    def _determine_action(self, event_type: str) -> str:
        ...
        if any(keyword in event_type for keyword in ["create", "activate", "add"]):
            ...
            return "update"
        elif any(keyword in event_type for keyword in ["delete", "deactivate", "remove"]):
            return "delete"
        else:
