...
class OktaBaseWebhookProcessor(AbstractWebhookProcessor):
    ...
    def __init__(self) -> None:
        ...
        # Allow tests to patch create_okta_client via user_webhook_processor module
        # Prefer a create_okta_client symbol from the specific subclass module (so tests can patch it)
        _create = create_okta_client
        try:
            mod = importlib.import_module(self.__class__.__module__)
            if hasattr(mod, "create_okta_client"):
                _create = getattr(mod, "create_okta_client")
        except Exception:
            pass
        self.client = _create()
    ...
    async def validate_payload(self, payload: EventPayload) -> bool:
        ...
        return (
            "eventType" in payload and "published" in payload and "eventId" in payload
        )
