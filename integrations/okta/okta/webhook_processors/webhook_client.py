...
class OktaWebhookClient(OktaEventHooksClient):
    ...
    async def create_webhook_if_not_exists(self, app_host: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"Successfully created event hook for URL: {webhook_url}"
            )
        except Exception as e:
            logger.error(
                f"Error creating event hook for URL {webhook_url}: {e}"
            )
            ...
            for hook in event_hooks:
                if hook.get("channel", {}).get("config", {}).get("uri") == webhook_url:
                    if hook_id := hook.get("id"):
                        ...
                        logger.info(f"Deleted event hook {hook_id} for URL: {webhook_url}")
                        return
