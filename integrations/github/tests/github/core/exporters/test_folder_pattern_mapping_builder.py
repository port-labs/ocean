...
@pytest.mark.asyncio
async def test_folder_pattern_mapping_builder_glob_repos() -> None:
    org_exporter = MagicMock()
    repo_exporter = MagicMock()
    ...
    # search_for_repositories should yield repos for patterns
    async def _search_gen(
        *args: Any, **kwargs: Any
    ) -> AsyncGenerator[List[Dict[str, Any]], None]:
        yield [
            {"name": "alpha-service", "default_branch": "main"},
            {"name": "alpha-worker", "default_branch": "main"},
            {"name": "beta-ui", "default_branch": "develop"},
        ]

    with patch(
        "github.helpers.repo_selectors.search_for_repositories", new=_search_gen
    ):
        builder = FolderPatternMappingBuilder(
            org_exporter, repo_exporter, repo_type="all"
        )
        selectors = [
            FolderSelector(
                ...
                path="configs/*",
                repos=[
                    RepositoryBranchMapping(name="alpha-*", branch=None),
                ],
            )
        ]

        result = await builder.build(selectors)
        # Should include both alpha-* repos, not beta-ui
        repo_names = sorted([opt["repo_name"] for opt in result])
        assert repo_names == ["alpha-service", "alpha-worker"]
        for opt in result:
            assert opt["folders"][0]["path"] == "configs/*"
            # Branch should fallback to each repo default branch
            assert opt["folders"][0]["branch"] in ("main", "develop")
