...
# Test data constants
MEMBER_PAGE_SIZE_IN_EXPORTER = 50

# Team Alpha Members
TEAM_ALPHA_MEMBERS_PAGE1_NODES = [
    {"login": f"member_alpha_{i+1}", "email": f"member_alpha_{i+1}@example.com", "isSiteAdmin": False}
    for i in range(MEMBER_PAGE_SIZE_IN_EXPORTER)
]
TEAM_ALPHA_MEMBERS_PAGE1_PAGEINFO = {"hasNextPage": True, "endCursor": "cursor_alpha_p1"}

TEAM_ALPHA_MEMBERS_PAGE2_NODES = [
    {"login": "member_alpha_last", "email": "member_alpha_last@example.com", "isSiteAdmin": False}
]
TEAM_ALPHA_MEMBERS_PAGE2_PAGEINFO = {"hasNextPage": False, "endCursor": "cursor_alpha_p2"}

TEAM_ALPHA_ALL_MEMBERS_NODES = TEAM_ALPHA_MEMBERS_PAGE1_NODES + TEAM_ALPHA_MEMBERS_PAGE2_NODES

# Team Alpha - Initial state (as fetched by first GQL call)
TEAM_ALPHA_INITIAL = {
    "id": "T_ALPHA",
    "slug": "team-alpha",
    "name": "Team Alpha",
...
TEAM_BETA_MEMBER_NODES = [{"login": "member_beta_1", "email": "member_beta_1@example.com", "isSiteAdmin": True}]
TEAM_BETA_INITIAL = {
    "id": "T_BETA",
    "slug": "team-beta",
    "name": "Team Beta",
...
class TestGraphQLTeamExporter:
    async def test_get_resource_with_member_pagination(
        ...
        # Mock send_api_request to simulate fetching a team with paginated members
        mock_send_api_request = MagicMock(
            side_effect=[
                # First call (initial team fetch)
                MagicMock(
                    spec=httpx.Response,
                    status_code=200,
                    json=lambda: GET_RESOURCE_TEAM_ALPHA_PAGE1_RESPONSE,
                ),
                # Second call (fetch_other_members)
                MagicMock(
                    spec=httpx.Response,
                    status_code=200,
        ...
        with patch.object(
            ...
            # Assertions for the first call (initial team fetch)
    ...
    async def test_get_resource_no_member_pagination(
        ...
        exporter = GraphQLTeamExporter(graphql_client)
        with patch.object(
            graphql_client, "send_api_request", return_value=mock_response_beta
        ) as mock_request:
    ...
    async def test_get_paginated_resources_with_member_pagination(
        ...
        event_context.set(
            mock_port_app_config, event_context.get().event_id, event_context.get().trigger_type
        )
        # Teams data where one team (Alpha) has paginated members
        ...
        with patch.object(
        ...
        ) as mock_gql_paginated_req, patch.object(
            exporter, "fetch_other_members", new=mock_fetch_other_members
            ...
            # Check Team Alpha (was paginated)
            assert result_batches[0][0] == TEAM_ALPHA_RESOLVED
            # Check Team Beta (was not paginated)
            ...
            expected_variables_for_teams_fetch = {
                ...
                "memberFirst": MEMBER_PAGE_SIZE_IN_EXPORTER,
                # 'first' for team pagination is handled by send_paginated_request itself
            }
