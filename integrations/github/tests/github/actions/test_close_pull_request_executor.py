...
PR_RESPONSE = {
    ...
    "state": "closed",
    "merged": False,
    "closed_at": "2026-09-07T09:41:20Z",
}
...
def patched_ocean() -> Generator[MagicMock, None, None]:
    ...
    with patch("github.actions.close_pull_request_executor.ocean") as mock_ocean:
...
class TestClosePullRequestExecutor:
    ...
    async def test_malformed_response(
        ...
        with pytest.raises(ClosePullRequestError, match="empty or incomplete"):
            await executor.execute(run)
