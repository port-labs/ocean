...
from github.actions.create_pr_executor import (
    CreatePrExecutor,
...
def make_run(execution_properties: dict[str, Any]) -> ActionRun:
    return ActionRun(
        ...
        action=ActionRun.Action(identifier="create_pr"),
        payload=IntegrationActionInvocationPayload(
            type="INTEGRATION_ACTION",
            installationId="inst-1",
            integrationActionType="create_pr",
...
def patched_ocean() -> Generator[MagicMock, None, None]:
    ...
    with patch("github.actions.create_pr_executor.ocean") as mock_ocean:
...
) -> Generator[CreatePrExecutor, None, None]:
    with patch(
        "github.actions.create_pr_executor.create_github_client_for_org",
        new=AsyncMock(return_value=mock_rest_client),
    ):
        yield CreatePrExecutor()


class TestCreatePrExecutor:
    @pytest.mark.asyncio
    async def test_happy_path(
        ...
        mock_port_client.post_run_log.assert_called_once()
        assert CREATING_STATUS_LABEL in mock_port_client.post_run_log.call_args.kwargs.get(
            "status_label", mock_port_client.post_run_log.call_args.args[1] if len(mock_port_client.post_run_log.call_args.args) > 1 else ""
        ) or mock_port_client.post_run_log.call_args.kwargs.get("status_label") == CREATING_STATUS_LABEL
    ...
    async def test_missing_required_inputs(
        self, executor: CreatePrExecutor
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_partition_key(self, executor: CreatePrExecutor) -> None:
        run = make_run({
            "org": "port-labs",
            "repo": "ocean",
            "title": "t",
