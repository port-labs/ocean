...
class MockGitHubClient:
    ...
    async def make_request(self, resource: str, simulate_rate_limit: bool = False):
        ...
        async with self.rate_limiter:
            ...
            self.rate_limiter.update_rate_limits(mock_response.headers, resource)
...
class TestRateLimiterIntegration:
    ...
    @pytest.mark.asyncio
    async def test_successful_requests_update_rate_limits(self, client_config, github_host, mock_sleep):
    ...
    @pytest.mark.asyncio
    async def test_rate_limit_error_pauses_requests(self, client_config, github_host, mock_sleep):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_requests_respect_rate_limits(self, client_config, github_host, mock_sleep):
    ...
    @pytest.mark.asyncio
    async def test_registry_returns_same_limiter_for_same_host_and_type(self, client_config, github_host, mock_sleep):
    ...
    @pytest.mark.asyncio
    async def test_rate_limit_status_monitoring(self, client_config, github_host, mock_sleep):
        ...
        for i in range(3):
            ...
            assert status[client_config.api_type]["utilization_percentage"] == ((i + 1) / 1000) * 100

    @pytest.mark.asyncio
    async def test_context_manager_handles_exceptions(self, client_config, github_host, mock_sleep):
    ...
    @pytest.mark.asyncio
    async def test_context_manager_re_raises_non_rate_limit_errors(self, client_config, github_host, mock_sleep):
    ...
    @pytest.mark.asyncio
    async def test_context_manager_handles_successful_requests(self, client_config, github_host, mock_sleep):
    ...
    @pytest.mark.asyncio
    async def test_different_api_types_have_separate_limits(self, github_host, mock_sleep):
        """Test that different API types have separate rate limits."""
        # Create different client configurations
        rest_config = GitHubRateLimiterConfig(api_type="rest", max_retries=3, max_concurrent=5)
        graphql_config = GitHubRateLimiterConfig(api_type="graphql", max_retries=2, max_concurrent=3)
    ...
    @pytest.mark.asyncio
    async def test_concurrency_limits_are_respected(self, client_config, github_host, mock_sleep):
    ...
    @pytest.mark.asyncio
    async def test_rate_limit_with_different_backoff_times(self, client_config, github_host, mock_sleep):
        ...
        for retry_after, expected_backoff in test_cases:
            ...
            response = await custom_client.make_request("/user", simulate_rate_limit=True)
            ...
            assert mock_sleep.call_count >= 1
            # Check that one of the calls was with the expected backoff time
            assert any(call.args[0] == expected_backoff for call in mock_sleep.call_args_list)

    @pytest.mark.asyncio
    async def test_http_client_retry_logic_with_rate_limiter(self, client_config, github_host, mock_sleep):
        """Test how real HTTP clients would implement retry logic with rate limiter."""
        class RealisticMockGitHubClient(MockGitHubClient):
            async def make_request_with_retry(self, resource: str, max_retries: int = 3):
                """Simulate how real HTTP clients would handle rate limiting with retries."""
                for attempt in range(max_retries + 1):
                    try:
                        response = await self.make_request(resource, simulate_rate_limit=(attempt == 0))
    ...
    @pytest.mark.asyncio
    async def test_rate_limit_pause_blocks_other_coroutines(self, mock_sleep, github_host):
        config = GitHubRateLimiterConfig(api_type="rest", max_concurrent=5, max_retries=0)
        limiter = GitHubRateLimiterRegistry.get_limiter(github_host, config)

        block_order = []

        async def rate_limited_task(name, delay_before_enter=0, trigger_rate_limit=False):
        ...
        task_a = asyncio.create_task(rate_limited_task("A", delay_before_enter=0, trigger_rate_limit=True))
