...
class TestAbstractGithubClient:
    ...
    async def test_wait_if_resync_threshold_exceeded_sleeps(
        self, authenticator: AbstractGitHubAuthenticator
    ) -> None:
        client = ConcreteGithubClient(
            organization="test-org",
    ...
    async def test_wait_if_resync_threshold_below_threshold_does_not_sleep(
        self, authenticator: AbstractGitHubAuthenticator
    ) -> None:
        client = ConcreteGithubClient(
            organization="test-org",
        ...
        # 10% utilization is below the 95% threshold, so it should NOT sleep
        client.rate_limiter.rate_limit_info = MagicMock(
            utilization_percentage=10.0, seconds_until_reset=10
        )
    ...
    async def test_wait_if_resync_threshold_ignored_for_non_resync(
        ...
        # Even if utilization is high, it should NOT sleep
        client.rate_limiter.rate_limit_info = MagicMock(
            utilization_percentage=99.0, seconds_until_reset=10
        )

        with patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
            # The event context is not a RESYNC event, so it should not sleep
            async with event_context(EventType.HTTP_REQUEST, trigger_type="machine"):
                await client._wait_if_resync_threshold_exceeded()

            mock_sleep.assert_not_called()

    async def test_wait_if_resync_threshold_ignored_when_outside_any_event_context(
        self, authenticator: AbstractGitHubAuthenticator
    ) -> None:
        client = ConcreteGithubClient(
            organization="test-org",
