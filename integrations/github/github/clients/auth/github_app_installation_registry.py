...
class GitHubAppInstallationRegistry:
    ...
    async def _ensure_index(cls, config: dict[str, Any]) -> dict[str, AuthScope]:
        ...
        jwt_client = GitHubAppJwtClient.from_config(config)
        ...
        if installation_id:
            ...
            account_type = None
            if not organization:
                installation = await jwt_client.fetch_installation(str(installation_id))
        ...
        async for page in jwt_client.iter_app_installations():
