...
class AllRepositorySelector(RepositorySelectorStrategy):
    ...
    async def select_repos(
        ...
        logger.info(f"Fetching all '{self.repo_type}' repositories from '{org_login}' of type '{org_type}'.")
        options = ListRepositoryOptions(organization=org_login, organization_type=org_type, type=self.repo_type)
...
class OrganizationLoginAndTypeGenerator:
    ...
    async def __call__(self, organization: Optional[str]) -> AsyncGenerator[Tuple[str, str], None]:
        port_app_config = cast("GithubPortAppConfig", event.port_app_config)
        org_options: ListOrganizationOptions = {"allow_personal_organization": port_app_config.allow_personal_organization}
        if organization:
