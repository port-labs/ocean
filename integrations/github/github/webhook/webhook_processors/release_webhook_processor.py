...
from github.webhook.events import RELEASE_DELETE_EVENTS, RELEASE_EVENTS
...
class ReleaseWebhookProcessor(BaseRepositoryWebhookProcessor):
    async def _validate_payload(self, payload: EventPayload) -> bool:
        ...
        return (
            action in RELEASE_EVENTS
            and "release" in payload
            and "id" in payload["release"]
        )
    ...
    async def handle_event(
        ...
        logger.info(f"Processing release event: {action} for release {release_id} in {repo_name}")

        if action in RELEASE_DELETE_EVENTS:
            data_to_delete = {"release": {**release, "name": release["name"].replace(" ", "_")}, "repo": {"name": repo_name} }
