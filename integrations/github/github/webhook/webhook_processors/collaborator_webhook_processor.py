...
class CollaboratorMemberWebhookProcessor(BaseRepositoryWebhookProcessor):
    ...
    async def _should_process_event(self, event: WebhookEvent) -> bool:
        return (
            event.headers.get("x-github-event") == self.EVENT_NAME
...
class CollaboratorMembershipWebhookProcessor(BaseRepositoryWebhookProcessor):
    ...
    async def validate_payload(self, payload: EventPayload) -> bool:
        ...
        return all(
            key in entity
            for key, entity in [("login", org), ("name", team), ("login", member)]
        )
    ...
    async def handle_event(
        ...
        if action not in COLLABORATOR_UPSERT_EVENTS:
            logger.info(f"Skipping unsupported membership event {action}")
            return WebhookEventRawResults([], [])
        ...
        enriched = self._enrich_collaborators_with_repositories(member, repositories)
        return WebhookEventRawResults(
            updated_raw_results=enriched, deleted_raw_results=[]
        )

    def _enrich_collaborators_with_repositories(
        self, member: Dict[str, Any], repositories: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        return [
            enrich_with_repository(member.copy(), repo["name"]) for repo in repositories
        ]
...
class CollaboratorTeamWebhookProcessor(BaseRepositoryWebhookProcessor):
    ...
    async def validate_payload(self, payload: EventPayload) -> bool:
        org = payload.get("organization", {})
        team = payload.get("team", {})
        return "login" in org and "name" in team
