...
class BranchWebhookProcessor(BaseRepositoryWebhookProcessor):
    ...
    async def handle_event(
        ...
        logger.info(f"Processing branch event: {self._event_type} for branch {branch_name} in {repo_name}")

        if self._event_type == "delete":
            data_to_delete = {"branch": {"name": branch_name}, "repository": {"name": repo_name}}
