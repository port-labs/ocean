...
async def resync_repositories(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        tasks = (
            RestRepositoryExporter(
                create_github_client(org["login"])
...
async def resync_users(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        tasks = (
            GraphQLUserExporter(
                create_github_client(org["login"], GithubClientType.GRAPHQL)
            ).get_paginated_resources()
            for org in organizations
        )
...
async def resync_teams(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            ...
            if selector.members:
                exporter = GraphQLTeamWithMembersExporter(
                    create_github_client(org_name, GithubClientType.GRAPHQL)
                )
            else:
                exporter = RestTeamExporter(
                    create_github_client(org_name, GithubClientType.REST)
                )
            tasks.append(exporter.get_paginated_resources(None))
...
async def resync_workflows(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        tasks = []
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            repo_exporter = RestRepositoryExporter(client)
            workflow_exporter = RestWorkflowExporter(client)

            repo_options = ListRepositoryOptions(type=port_app_config.repository_type)
...
async def resync_pull_requests(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            repository_exporter = RestRepositoryExporter(client)
            pull_request_exporter = RestPullRequestExporter(client)
...
async def resync_issues(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            repository_exporter = RestRepositoryExporter(client)
            issue_exporter = RestIssueExporter(client)
...
async def resync_releases(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            repository_exporter = RestRepositoryExporter(client)
            release_exporter = RestReleaseExporter(client)
...
async def resync_tags(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            repository_exporter = RestRepositoryExporter(client)
            tag_exporter = RestTagExporter(client)
...
async def resync_branches(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            repository_exporter = RestRepositoryExporter(client)
            branch_exporter = RestBranchExporter(client)
...
async def resync_dependabot_alerts(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            repository_exporter = RestRepositoryExporter(client)
            dependabot_alert_exporter = RestDependabotAlertExporter(client)
...
async def resync_code_scanning_alerts(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            repository_exporter = RestRepositoryExporter(client)
            code_scanning_alert_exporter = RestCodeScanningAlertExporter(client)
...
async def resync_folders(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            folder_exporter = RestFolderExporter(client)

            repo_path_map = create_path_mapping(selector.folders)
            folder_options = ListFolderOptions(repo_mapping=repo_path_map)

            tasks.append(folder_exporter.get_paginated_resources(folder_options))

        async for folders in stream_async_iterators_tasks(*tasks):
            yield folders
...
async def resync_files(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            file_exporter = RestFileExporter(client)
            repo_exporter = RestRepositoryExporter(client)

            repo_path_map = await group_file_patterns_by_repositories_in_selector(
                files_pattern, repo_exporter, port_app_config.repository_type
            )

            tasks.append(file_exporter.get_paginated_resources(repo_path_map))
...
async def resync_collaborators(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            repository_exporter = RestRepositoryExporter(client)
            collaborator_exporter = RestCollaboratorExporter(client)
...
async def resync_secret_scanning_alerts(kind: str) -> ASYNC_GENERATOR_RESYNC_TYPE:
    ...
    async for organizations in org_exporter.get_paginated_resources(org_options):
        ...
        for org in organizations:
            client = create_github_client(org["login"], GithubClientType.REST)
            repository_exporter = RestRepositoryExporter(client)
            secret_scanning_alert_exporter = RestSecretScanningAlertExporter(client)
