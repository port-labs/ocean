import httpx
from typing import cast

from initialize_client import init_client
from integration import ObjectKind
from datadog.overrides import TeamResourceConfig
from port_ocean.core.handlers.port_app_config.models import ResourceConfig
from port_ocean.core.handlers.webhook.webhook_event import (
    EventPayload,
    WebhookEvent,
    WebhookEventRawResults,
)

from datadog.core.exporters import TeamExporter
from datadog.core.exporters.team_exporter import GetTeamOptions
from datadog.webhook.webhook_processors.audit_trails.base_processor import (
    BaseProcessor,
)


class TeamWebhookProcessor(BaseProcessor):
    async def should_process_event(self, event: WebhookEvent) -> bool:
        return (
            await super().should_process_event(event)
            and self.extract_asset_type(event.payload) == ObjectKind.TEAM
        )

    async def get_matching_kinds(self, _: WebhookEvent) -> list[str]:
        return [ObjectKind.TEAM]

    async def handle_event(
        self, payload: EventPayload, resource_config: ResourceConfig
    ) -> WebhookEventRawResults:
        team_id = self.extract_asset_id(payload)
        if team_id is None:
            return WebhookEventRawResults(
                updated_raw_results=[], deleted_raw_results=[]
            )

        if self.is_delete_event(payload):
            return WebhookEventRawResults(
                updated_raw_results=[],
                deleted_raw_results=[{"id": team_id}],
            )

        config = cast(TeamResourceConfig, resource_config)
        dd_client = init_client()
        team_exporter = TeamExporter(dd_client)
        try:
            team = await team_exporter.get_resource(
                GetTeamOptions(
                    id=team_id,
                    include_members=config.selector.include_members,
                )
            )
        except httpx.HTTPStatusError as err:
            if err.response.status_code == 404:
                return WebhookEventRawResults(
                    updated_raw_results=[],
                    deleted_raw_results=[{"id": team_id}],
                )
            raise

        return WebhookEventRawResults(
            updated_raw_results=[team] if team else [],
            deleted_raw_results=[],
        )

    async def validate_payload(self, payload: EventPayload) -> bool:
        return self.extract_asset_id(payload) is not None
