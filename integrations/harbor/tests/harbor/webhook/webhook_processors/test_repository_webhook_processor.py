"""Tests for Harbor repository webhook processor."""
...
class TestRepositoryWebhookProcessor:
    """Test RepositoryWebhookProcessor."""
    ...
    @pytest.fixture
    def processor(self, mock_webhook_event: WebhookEvent) -> RepositoryWebhookProcessor:
        """Create processor instance."""
        with patch(
            "harbor.webhook.webhook_processors.repository_webhook_processor.init_client"
        ):
            return RepositoryWebhookProcessor(mock_webhook_event)
    ...
    async def test_validate_payload_valid_event(
        self, processor: RepositoryWebhookProcessor, mock_payload: Dict[str, Any]
    ) -> None:
    ...
    async def test_handle_event_delete_artifact(
        ...
        assert result.deleted_raw_results[0]["name"] == "test/repo"
