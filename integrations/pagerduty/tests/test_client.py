...
class TestPagerDutyClient:
    async def test_paginate_request_to_pager_duty(self, client):
        ...
        with patch("port_ocean.utils.http_async_client.request", side_effect=mock_responses):
            collected_data = []
    ...
    async def test_create_webhooks_if_not_exists(self, client):
        ...
        with patch("port_ocean.utils.http_async_client.request", side_effect=[
            no_webhook_response, 
        ...
        with patch("port_ocean.utils.http_async_client.request", return_value=existing_webhook_response):
            await client.create_webhooks_if_not_exists()
    ...
    async def test_update_oncall_users(self, client):
        ...
        with patch("port_ocean.utils.http_async_client.request", return_value=mock_response):
            ...
            for service in updated:
                ...
                matching_oncalls = [
                    oncall for oncall in TEST_DATA["oncalls"]
                    if oncall["escalation_policy"]["id"] == service["escalation_policy"]["id"]
                ]
    ...
    async def test_send_api_request(self, client):
        ...
        with patch("port_ocean.utils.http_async_client.request", return_value=not_found_response):
    ...
    async def test_transform_user_ids_to_emails(self, client):
        ...
        def mock_get_cached_user(user_id):
            user_map = {
                "PU123": "user1@example.com",
                "PU456": "user2@example.com"
            }
            return user_map.get(user_id)
        
        with patch.object(client, 'fetch_and_cache_users', side_effect=mock_fetch_and_cache_users):
            with patch.object(client, 'get_cached_user', side_effect=mock_get_cached_user):
