...
class WebhookHandler:
    def __init__(self, gitlab_handler: GitlabClient):
        self.gitlab_handler = gitlab_handler

    async def setup_group_webhooks(self, webhook_url: str, events: List[str]) -> None:
        async for page in self.gitlab_handler.get_paginated_resources("groups"):
            for group in page:
                ...
                try:
                    ...
                    existing_webhooks = await self.gitlab_handler.make_request(
                        f"{self.gitlab_handler.gitlab_host}/groups/{group_id}/hooks")
                    webhook_exists = any(
                    ...
                    if not webhook_exists:
                        await self.setup_group_webhook(group_id, webhook_url, events)
    ...
    async def setup_group_webhook(self, group_id: str, webhook_url: str, events: List[str]):
        try:
            # Check if webhook already exists
            existing_webhooks = await self.gitlab_handler.make_request(
                f"{self.gitlab_handler.gitlab_host}/groups/{group_id}/hooks")
            ...
            response = await self.gitlab_handler.make_request(
                f"{self.gitlab_handler.gitlab_host}/groups/{group_id}/hooks", method="POST", json_data=webhook_data)
    ...
    async def delete_group_webhook(self, group_id: str, webhook_id: str):
        try:
            await self.gitlab_handler.make_request(f"groups/{group_id}/hooks/{webhook_id}", method="DELETE")
