...
for folder in "$(pwd)"/integrations/*; do
    if [ -d "$folder" ]; then
        ...
        (cd "$folder" && source .venv/bin/activate && towncrier build --no --version $new_version && rm changelog/1.improvement.md && git add . && git commit -m "Bumped ocean version to $VERSION for $(basename "$folder")")
    fi
done