...
#   scripts/generate-release-intents.sh patch improvement "Bumped ocean core" --exclude fake-integration
...
_usage() {
  cat <<EOF
Usage: ${SCRIPT_NAME} <bump> <changelog-type> <changelog...> [--only NAME ...] [--exclude NAME ...] [--name FILE]
...
for integration in "${SELECTED[@]}"; do
  dest_dir="${ROOT_DIR}/integrations/${integration}/.ocean-release"
  dest_file="${dest_dir}/${NAME}.yaml"
  mkdir -p "${dest_dir}"
