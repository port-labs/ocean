...
class BaseIntegration:
    ...
    def __init__(self, config: IntegrationConfiguration):
        self._manipulation, self._transport, self._port_app_config, self._port_client = None, None, None, None
        self.started = False
        self.config = config
    ...
    async def _init_port_client_instance(self):
        self._port_client = self.PortClientClass(self.config.port.client_id, self.config.port.client_secret,
                                                 self.config.port.base_url, self.config.integration.identifier)
        return self._port_client
    ...
    async def register_state(self, kind: str, entities_state: Change):
        resource_mappings = [resource for resource in self.port_app_config.resources
                             if resource.kind == kind]

        parsed_entities = [self.manipulation.get_entities_diff(mapping, [entities_state])
    ...
    async def trigger_start(self):
        ...
        port_client.initiate_integration(self.config.integration.identifier, self.config.integration.type,
    ...
    async def trigger_resync(self):
        ...
        parsed_entities = [self.manipulation.get_entities_diff(mappings, results)
